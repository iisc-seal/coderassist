...
public class MainDriver {
	...
	public static void main(String[] args) {
		...
		if (args.length != 2) {
			...
			System.out.println("Input subsFile and path to store logs\n");
			return;
		}
...
//		readReferences(refList, refsScanner, Inps);
		...
		while (subsScanner.hasNext()) {
			...
			String currentline = subsScanner.next();
			String[] progs = currentline.split(",");
			String currentSub = progs[0], currentRef = progs[1];
			
			if (subsToSkip.contains(currentSub) || subsToSkip.contains(currentRef))
				continue;
			System.out.println("Processing -> "+ currentSub + " with " + currentRef);
			
//			for(SummaryModule referenceSummary: refList){
//				System.out.println("Matching against "+referenceSummary.getName());
			SummaryModule referenceSummary = readRef(currentRef, Inps);
...
//			}
			...
			System.out.println("Took "+ (refEndTime - refStartTime)/Math.pow(10, 9) + " on" + currentSub);
...
//		}		
	...
	private static void readReferences(List<SummaryModule> refList,
		...
		while (scr.hasNext()) {
			...
			UpdateSummary opSum = new UpdateSummary(fileName, fileName + ".output.summary", ErrorLogger);
			ret = opSum.parse();
			if (!ret) {
				System.out.println("Failed to parse "+ opSum.getProgUpdateSummaryName());
				cleanLoggers();
				continue;
			}
	...
	private static SummaryModule readRef(String refFileName, ProgramInputs Inps) {
			String fileName = FeedbackSummary.getPrefix(refFileName);
			...
			return new SummaryModule(typeSum, inputSum, initSum, updateSum, opSum, fileName);
	}
