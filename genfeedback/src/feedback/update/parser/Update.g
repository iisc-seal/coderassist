...
prog 
  :  (Update|'INIT:'|'INPUT:'|'OUTPUT:') block EOF -> block
  ;
...
loopStatement
 // Uncomment when summary generation is fixed
  : Loop '(' expression Comma expression Comma expression Comma expression ')' '[' expression ']'
    '{' statement* '}' -> ^(LOOP ^(DIR expression) ^(INDEX expression) ^(LB expression) ^(UB expression) ^(LINE expression) ^(BODY statement*))
//    : Loop '(' expression Comma expression Comma expression ')' 
//    '{' statement* '}' -> ^(LOOP ^(INDEX expression) ^(LB expression) ^(UB expression) ^(BODY statement*))
  ;
...
assignment
  : Id indexes? '=' expression -> ^(ASSIGNMENT ^(LHS Id indexes?) ^(RHS expression))   
  ;
